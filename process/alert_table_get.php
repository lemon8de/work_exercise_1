...
if ($stmt->rowCount() > 0) {
    foreach($stmt->fetchAll() as $x) {
        echo '
        <tr id="' . $x['id'] . '" onclick="alert_table_click.call(this)" style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#alert_table_click_modal" custom-content="' . htmlspecialchars($x["content"]) .  '">
            <td>' . $x["id_number"] . '</td>
            <td>' . $x["full_name"] . '</td>
            <td>' . $x["from_user"] . '</td>
